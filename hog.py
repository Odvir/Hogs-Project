...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    dice:            A function that simulates a single dice roll outcome.
    """
    # Leave these asserpython3 ok -q 03t statements here; they help check for errors.
    assert type(num_rolls) == int, 'num_rolls must be an integer.'
...
def GCD(player_score, opponent_score):
    ...
    while start <=end:
        if player_score % start == 0 and opponent_score % start == 0:
            GCD = start
        start+=1
    return GCD
...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    result = opponent_score - player_score
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def commentary(score0,score1):
        running_high_empty = running_high
        last_score_empty = last_score
        if who == 0:
            last_score_empty,running_high_empty = checks_running_high(score0,last_score_empty,running_high_empty,who)           
        else:
            last_score_empty,running_high_empty = checks_running_high(score1,last_score_empty, running_high_empty,who)
        return announce_highest(who,last_score_empty, running_high_empty)
    return commentary

def checks_running_high(score,last_score_empty,running_high_empty, who):
    if running_high_empty < score - last_score_empty:
        running_high_empty = score - last_score_empty
        print(running_high_empty, "point(s)! The most yet for Player",who)
    return score, running_high_empty
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    result, max_result, final_rolls= 0,0,0
    get_result = make_averaged(roll_dice,trials_count)
    i = 1
    while i < 11:
        result = get_result(i,dice)
        if result > max_result:
            max_result,final_rolls = result,i
        elif result == max_result:
            final_rolls = min(final_rolls,i)
            print(final_rolls)
            if final_rolls == i:
                max_result = result
        i+=1
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    roll_zero = free_bacon(opponent_score)
    if roll_zero >= cutoff:
        return 0
    return num_rolls
    # END PROBLEM 10
